export default function Giselle(allCode) {
  ...
  var regData = regOpen + regAnySpace + regOpen + regVariableName + regOr + regText + regOr + regKeys + regCloseOne + regAnySpace + regOpen + regIn + regOr + regBy + regOr + regKeys + regCloseOneOrMore + regCloseOne;
  var regFullCommand = regOpen + regCommands + regOr + regData + regCloseOneOrMore;
  var regFullStorage = regOpen + regAnySpace + regOpen + regStorage + regCloseOne + regOpen + regData + regCloseOneOrMore + regAnySpace + regCloseOne;
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      if (reg.test(lines[i])) {
        ...
        for (j = 0; j < words.length - 1; j++) {
          ...
          (found == false) && (word == 'sql') && (GiselleCalc('require("sqlite").Database("/GiselleDB").run', '"sql"', 2, 0, '++') && DegreeUp()) && (found = true);
          ...
          (found == false) && (word == /^[a-zA-Z_]{1}[a-zA-Z0-9_]{0,}$/) && (GiselleCalc(Dependent('(nextWord=="will") || ((nextWord=="in") && ((words[j+3]=="will") || (words[j+6]=="will")))', 'var ' + word, Dependent('lastWord=="loop"', word, '*unknown command "' + word + '"')), '"' + word + '"', 2, 0, Dependent('lastWord=="by"', '-+', '++'))) && Dependent('arrayNow=="true"', 'code[i] += "]"', '') && (arrayNow = false) && (found = true);
